...
public class UserService {
	...
	private static JedisShardInfo info = new JedisShardInfo("http://192.168.10.214:6379/1");
	public String selectUserById(String userid, String pass) {
		JSONObject res = new JSONObject();
		try {
			User t = null;
			if (pass != null &&!"".equals(pass.trim())) {
			     //则字符串不为空或空格
				t = dao.selectUserByIdAndPass(userid, pass);
			}else {
				t = dao.selectUserById(userid, pass);
			}
			if (!(t == null)) {
				String img;
				try {
					img = ENCODER.encodeToString(t.getPhoto());
				} catch (NullPointerException e) {
					img = WARNPIC;
				}
				res.put("state", 0);
				res.put("msg", "查询用户信息成功！");
				res.put("data", t);
				res.put("img", img);
			} else {
				res.put("state", 100);
				res.put("msg", "查询用户信息失败！");
			}
		} catch (Exception e) {
			e.printStackTrace();
			res.put("state", -500);
		...
		logger.info("从"+this.getClass().getName()+"收到请求返回："+res.get("msg").toString());
		return res.toString();
	}
	public String iclassLogin(String userid, String pass) {
		JSONObject res = new JSONObject();
		try {
			User t = null;
			if (pass != null && !"".equals(pass.trim())) {
			...
			if (t == null && pass != null &&!"".equals(pass.trim())) {
				t = dao.selectUserByPhoneNumber(userid, pass);
			}
			if (!(t == null)) {
				res.put("state", 0);
				res.put("msg", "用户登录成功！");
	...
	public String iclassToken() {
		JSONObject res = new JSONObject();
		try {
		    String host = FtpUtils.getProperties("redis.properties", "tokenHost");

   			JedisShardInfo info = new JedisShardInfo(host);
   			Jedis jedis = new Jedis(info);
   			String token = UUID.randomUUID().toString();
   			jedis.hset("teaTokens", token, "");
   			res.put("token", token);
   			res.put("state", 0);
   			res.put("msg", "生成uuid成功！");
   			jedis.close();
		} catch (Exception e) {
			e.printStackTrace();
			res.put("state", -500);
			res.put("msg", "生成uuid错误！");
		}
		logger.info("从" + this.getClass().getName() + "收到请求返回：" + res.get("msg").toString());
		return res.toString();
	}
	public String scanForTeacher(String token, String userid) {
		JSONObject res = new JSONObject();
		try {
			Jedis jedis = new Jedis(info);
			try {
				String a = jedis.hget("teaTokens", token);
				if (a != null) {
					User user = dao.selectUserById(userid, "");
					if (user != null) {
						jedis.hset("teaTokens", token, userid);
						res.put("msg", "登录成功！");
						res.put("state", 0);
						res.put("data", user);
						MQUtil.sendToMQ("sacnForTeacher", "scanForTeacher", "keys", res.toString());
						res.remove("data");
						} else {
						res.put("msg", "登录失败，数据库无此userid！");
					}
	            } else {
            		res.put("msg", "登录失败，缓存中无此token！");
	            }
				jedis.close();
			} catch (Exception e) {
				e.printStackTrace();
				res.put("state", -500);
				res.put("msg", "登录失败，数据库错误！");
			}
		} catch (Exception e) {
			e.printStackTrace();
			res.put("msg", "缓存服务器连接失败！");
		}
	...
	public String selectUserByIdForAndroid(String userid) {
		User t = null;
		t = dao.selectUserById(userid, null);
		try {
			if (!(t == null)) {
	...
	public String selectAllUser(int page, int count) {
		JSONObject res = new JSONObject();
		try {
			List<User> t = null;
			t = dao.selectAllUser((page - 1) * count, count);
			if (!(t == null)) {
				res.put("state", 0);
				res.put("msg", "查询用户信息成功！");
				res.put("data", t);
	...
	public String updateUser(User user) {
		...
		try {
			int t = 0;
			t = dao.updateUser(user);
			if (!(t == 0)) {
				res.put("state", 0);
				res.put("msg", "更新用户信息成功！");
			} else {
				res.put("state", 100);
				res.put("msg", "更新用户信息失败！");
			}
		} catch (Exception e) {
			e.printStackTrace();
			res.put("state", -500);
	...
	public String saveCurrentsid(String sessionid,String userid) {
		JSONObject res = new JSONObject();
		try {
			int t = 0;
			t = dao.saveCurrentsid(sessionid,userid);
			if (!(t == 0)) {
				res.put("state", 0);
				res.put("msg", "修改配置成功！");
			} else {
	...
	public String updateUserPhoto(byte[] photo, String userid) {
		JSONObject res = new JSONObject();
		try {
			int t = 0;
			t = dao.updateUserPhoto(photo, userid);
			if (!(t == 0)) {
				res.put("state", 0);
				res.put("msg", "修改用户图片成功！");
			} else {
				res.put("state", 100);
				res.put("msg", "修改用户图片失败！");
			}
		} catch (Exception e) {
			e.printStackTrace();
			res.put("state", -500);
	 ...
	 * @return
	 */
	public String startIPCall(String ip) {
		JSONObject res = new JSONObject();
		try {
			User t = null;
			t = dao.startIPCall();
			...
			if (!(t == null)) {
				...
				res.put("msg", "查询配置成功！");
				res.put("data", t);
				dao.setIPCallStatus(t.getUserid(), new Date(), null,ip, 2);
			} else {
				res.put("state", 100);
				res.put("msg", "查询配置失败！");
			}
	 ...
	 * @return
	 */
	public String setIPCallStatus(String userid, Date start, Date end, String ip, Integer status) {
		JSONObject res = new JSONObject();
		...
		try {
			int t = 0, w = 0; // (start==null)?null:new Date(start)
			t = dao.updateIPCallstatus(start,status, ip);
			w = dao.updateOncallTo1(userid);
			if (t > 0 && w > 0) {
				res.put("state", 0);
				res.put("msg", "添加配置成功！");
				res.put("data", t);
	...
	// 修改 online=1
	public String updateOnlineTo1(String userid, String ip,String pass,Integer pcport) {
		JSONObject res = new JSONObject();
		try {
			int t = 0;
			t = dao.updateOnlineTo1(userid, ip,pass,pcport);
			if (!(t == 0)) {
				res.put("state", 0);
				res.put("msg", "修改配置成功！");
			} else {
	...
	// 修改online=0
	public String updateOnlineTo0(String userid,String pass) {
		JSONObject res = new JSONObject();
		try {
			int t = 0;
			t = dao.updateOnlineTo0(userid,pass);
			if (!(t == 0)) {
				res.put("state", 0);
				res.put("msg", "修改配置成功！");
			} else {
	...
	// 修改oncall=1
	public String updateOncallTo1(String userid) {
		JSONObject res = new JSONObject();
		try {
			int t = 0;
			t = dao.updateOncallTo0(userid);
			if (!(t == 0)) {
				res.put("state", 0);
				res.put("msg", "修改配置成功！");
			} else {
	...
	// 修改oncall=0
	public String updateOncallTo0(String userid) {
		JSONObject res = new JSONObject();
		try {
			int t = 0;
			t = dao.updateOncallTo0(userid);
	...
	public String endIPCall(Date date, String edeviceip, String pcip) {
		JSONObject res = new JSONObject();
		try {
			int t = 0, w = 0;
			t = dao.endIPCall(date, edeviceip);
			w = dao.updateOncallTo0(pcip);
			if (t != 0 && w != 0) {
				res.put("state", 0);
				res.put("msg", "修改配置成功！");
			} else {
	...
	public String selectAllUserCount() {
		JSONObject res = new JSONObject();
		try {
			int t = 0;
			t = dao.selectAllUserCount();
			if (!(t == 0)) {
				res.put("state", 0);
				res.put("msg", "查询配置成功！");
				res.put("data",t);
